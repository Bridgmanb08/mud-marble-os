...
interface AddLineItemsToInvoiceModalProps {
  invoiceId: string;
  projectId: string;
  onClose: () => void;
  onAdded: () => void;
...
// The one "pull line items into an invoice" picker -- estimate lines and
// approved change order lines together (they're the same kind of line item),
// with chips to narrow to just the estimate or a single change order.
// BuilderTrend's "Add from Estimates" flow: check the line items to bill,
...
export function AddLineItemsToInvoiceModal({ invoiceId, projectId, onClose, onAdded }: AddLineItemsToInvoiceModalProps) {
  const toast = useToast();
  const [items, setItems] = useState<InvoiceableItem[] | null>(null);
  ...
  useEffect(() => {
    api
      .get<InvoiceableItem[]>(`/projects/${projectId}/invoiceable-items`)
      .then((data) => {
        setItems(data);
        setRows(Object.fromEntries(data.map((i) => [i.id, { checked: false, pct: '0', amount: '0.00' }])));
      })
      .catch(() => toast('Failed to load line items', true));
  ...
  const sources = useMemo(() => Array.from(new Set((items ?? []).map((i) => i.source_label))), [items]);
  const visibleItems = useMemo(
    () => (items ?? []).filter((i) => sourceFilter === 'all' || i.source_label === sourceFilter),
    [items, sourceFilter]
  );
  ...
  async function handleCommit() {
    ...
    const toAdd = items
      ...
      .map((i, idx) => ({
        ...
        title: i.source_type === 'change_order' ? `${i.source_label}: ${i.title}` : i.title,
  ...
  return (
    <Modal title="Add line items to invoice" onClose={onClose} xxl resizable>
      <div className="ibt" style={{ fontSize: 13, textTransform: 'none', letterSpacing: 0, border: 'none', padding: 0, marginBottom: 6 }}>
        Estimate &amp; change order line items
      </div>
      <p style={{ fontSize: 12.5, color: 'var(--t2)', marginTop: 0, marginBottom: 12 }}>
        Select any line items from the estimate or its approved change orders and indicate the percentage or amount to invoice.
      </p>
      ...
      {sources.length > 1 && (
        <div className="filters" style={{ marginBottom: 12 }}>
          {['all', ...sources].map((src) => (
            <button key={src} type="button" className={`fb${sourceFilter === src ? ' on' : ''}`} onClick={() => setSourceFilter(src)}>
              {src === 'all' ? 'All' : src}
            </button>
          ))}
        </div>
      ...
      {items === null ? (
        ...
        <div className="empty">
          <div className="empty-t">No line items found for this project.</div>
          <div className="empty-s">Build out an estimate (or approve a change order with line items) first, then come back to invoice against it.</div>
        </div>
      ) : (
              ...
              {visibleItems.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td style={{ color: 'var(--t2)' }}>{item.source_label}</td>
                    <td>{item.title}</td>
